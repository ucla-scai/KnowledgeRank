...
void Rank::Populate_prob_test(int i, int t, int word, int doc, int start) {
    if (t < K) {
    ...
    else if (id_psi.find(word) == id_psi.end() || ks_word[word]) {
        int b = t-K;
        double sum = 0.0;
        for (int a=0; a<A; a++) {
        ...
        int b = t-K;
        double total = delta_pow_sums[b][0];
        pr_test[i] = ((((double)n_w[t][word]) + psi[b][word]*total) / ( ((double) n_w_dot[t]) + ((double)n_w_dot_test[t]) + total)) *
                     (((double) n_d_test[t][doc] + alpha) / (((double) (corpus_test[doc].size() - 1)) + ((double) visible_topics.size()) * alpha));
    }
...
void Rank::Populate_prob(int i, int t, int word, int doc, int start) {
    if (t < K) {
        ...
        int b = t-K;
        double total = delta_pow_sums[b][0];
        pr[i] = (((double) n_w[t][word] + psi[b][word]) / (((double) n_w_dot[t]) + 1)) *
                (((double) n_d[t][doc] + alpha) / (((double) (corpus[doc].size() - 1)) + ((double) visible_topics.size()) * alpha));
        //pr[i] = 0.2; //*= psi[b][word] * psi[b][word];
        stats.cnt_model++;
        stats.tot_model += pr[i];
    }
...
void Rank::Calculate_phi(){
    ...
    for (int t=0; t<T; t++) {
        ...
        if (t < K) {
            ...
            int b = t-K;
            for (int w=0; w<V; w++) {
                if (id_psi.find(w) == id_psi.end() || ks_word[w]) {
                    double sum = 0.0;
                    for (int a = 0; a < A; a++) {
