...
void Display_help(const string & exe) {
    ...
    cout << "-alg=<name> Algorithm used (rank|src|lda) [default: rank]\n\t";
    ...
    cout << "-psi=<file> RNN distribution of topics [defalut: psi.dat]\n\t";
    cout << "-vocab=<file> Vocabulary for psi [defalut: vocab.dat]\n\n";
}
//----------------------------------------------------------------------------------
//----------------------------------------------------------------------------------
int main(int argc, char* argv[]) {
    ...
    string psi_input = input.getCmdOption("-psi");
    if (!psi_input.empty()){
        psi = psi_input;
    }
    string vocab_input = input.getCmdOption("-vocab");
    if (!vocab_input.empty()){
        vocab = vocab_input;
    }
    string ks_input = input.getCmdOption("-ks");
    if (!ks_input.empty()){
    ...
    if (alg == rank) {
        ...
        if (P > 1) {
            rankParallel topic_model(options);
            topic_model.load();
            topic_model.gibbs();
            topic_model.save();
        }
        else {
            rank topic_model(options);
            topic_model.load();
            topic_model.gibbs();
            topic_model.save();
        }
